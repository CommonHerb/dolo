...
- sources without top-level declarations, duplicate declarations, including
  ...
  parameter missing-name commas, function parameter trailing commas, missing
...
- `experiments/herbert/builtin_kind_candidate.herb` enumerates the same
  ...
  `HERBERT_BUILTIN_KINDS`, so the first built-in kind migration candidate is
  compared against the bootstrap table it intends
  to replace
...
Python-owned `HERBERT_BUILTIN_KINDS`, while `HERBERT_VALUE_BUILTINS` and
`HERBERT_VOID_BUILTINS` are derived from that table. The boolean operator
candidate has one extra local check: its Herbert lookup table must mirror
