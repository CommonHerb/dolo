...
"""
    ...
    def test_manifest_validator_requires_migration_candidate_python_owner(self):
        from dolo.manifests import ManifestError, validate_repository_manifests
...
"""
            ...
            (notes / "0001-candidate.md").write_text(
                "experiments/herbert/candidate.herb\n"
                "tests/fixtures/candidate.stdout\n"
                "This candidate is executable but does not say what Python behavior it mirrors.\n"
            )
            (fixtures / "executable_manifest.tsv").write_text(
                "examples/a.dolo\ttests/fixtures/a.herb\t"
                "tests/fixtures/a.stdout\n"
            ...
            with self.assertRaisesRegex(
                ManifestError,
                r"herbert_migration_manifest.tsv: migration candidate note must name "
                r"the current Python/bootstrap owner for experiments/herbert/candidate.herb",
            ):
                validate_repository_manifests(root)

    def test_manifest_validator_requires_migration_candidate_replacement_path(self):
        from dolo.manifests import ManifestError, validate_repository_manifests
...
"""
            ...
            with self.assertRaisesRegex(
                ManifestError,
                r"herbert_migration_manifest.tsv: migration candidate note must include "
                r"a replacement path for experiments/herbert/candidate.herb",
            ):
                validate_repository_manifests(root)
...
"""
            ...
            (fixtures / "a.herb").write_text("func main():\n  return 1\nend\n")
            (fixtures / "a.stdout").write_text("1\n")
            (experiments / "boolean_operator_candidate.herb").write_text(
                """func boolean_operator(name):
    if equal(name, "&&"):
        return "and"
    else:
        return "missing"
...
"""
            ...
            (fixtures / "boolean_operator_candidate.stdout").write_text("\"and\"\n")
            (notes / "0001-boolean.md").write_text(
...
"""
    ...
    def test_manifest_validator_rejects_stale_migration_candidate_stdout_notes(self):
        from dolo.manifests import ManifestError, validate_repository_manifests
...
"""
            ...
            (notes / "0002-stale.md").write_text(
                "experiments/herbert/candidate.herb\n"
                "tests/fixtures/stale-candidate.stdout\n"
            )
            (fixtures / "executable_manifest.tsv").write_text(
                "examples/a.dolo\ttests/fixtures/a.herb\t"
                "tests/fixtures/a.stdout\n"
            ...
            with self.assertRaisesRegex(
                ManifestError,
                r"herbert_migration_manifest.tsv: migration candidate note "
                r"is not linked to a manifest source/stdout pair: "
                r"docs/migration-candidates/0002-stale.md",
            ):
                validate_repository_manifests(root)
